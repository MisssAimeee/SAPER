## Gemini API inference code for RAPM
## Modified from original LLM_inference.py to use Google Gemini API
import json
...
def api_inference(RAG_prompt, model):
    ...
    # Safety settings to avoid blocking scientific protein content
    safety_settings = [
        {
    ...
    generation_config = genai.GenerationConfig(
        temperature=0.7,
        top_p=0.9,
        max_output_tokens=8192,  # Increased to handle longer protein annotations
    )
    ...
    try:
        ...
        # Check finish reason and warn if needed
...
if __name__ == "__main__":
    ...
    print("now task: ", now_task, "now k: ", now_k, "model: ", model)
    print("now task: ", now_task, "now k: ", now_k, "model: ", model, file=result_file)
    ...
    for i in range(len(all_answers)):
        ...
        with open(f"GEMINI_{now_task}_{now_k}_results.json", 'a+') as f:
